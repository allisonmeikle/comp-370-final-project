...
def parse_ctv(html_path):
    ...
    for card in soup.select("div.Card_dh-card-text-container__HrQl6"):
        # title
        title_tag = card.select_one("a.Card_dh-card-title__aTHbv")
        title = title_tag.get_text(strip=True) if title_tag else ""

        # excerpt
        excerpt_tag = card.select_one("div[data-testid='excerpt-container'] p")
        excerpt = excerpt_tag.get_text(strip=True) if excerpt_tag else ""

        # date
        date_tag = card.select_one("li.Card_dh-card-date__jBrq7")
        date = date_tag.get_text(strip=True) if date_tag else ""

        rows.append([title, excerpt, "dailyhive.com", date])
